-- Retention Rate dynamics during the first week after registration,
-- for all new users in Saransk.
WITH new_users AS (
...
SELECT
    ...
    ROUND(
        ...
        / MAX(COUNT(DISTINCT user_id)) OVER (ORDER BY day_since_install),
        2
    ) AS retention_rate
